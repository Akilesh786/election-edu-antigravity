...
.hero h1 {
    font-size: 2.8rem;
    font-weight: 800;
    background: linear-gradient(90deg, #f5a623, #ffd580, #f5a623);
    background-size: 200%;
...
.result-detail { color: #94a3b8; font-size: 0.95rem; }
...
.step-detail { color: #64748b; font-size: 0.88rem; margin-top: 6px; line-height: 1.6; }
...
/* ── Inputs ── */
.stNumberInput input, .stSelectbox select, .stRadio {
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 20px 0 10px;'>
      ...
      <div style='font-size:0.78rem; color:#475569; margin-top:3px;'>Education & Civic Guide</div>
    </div>
    <hr style='border-color:rgba(245,166,35,0.2); margin: 14px 0;'>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <hr style='border-color:rgba(245,166,35,0.2); margin: 20px 0 14px;'>
    <div style='font-size:0.78rem; color:#334155; text-align:center; line-height:1.6;'>
...
if page == "🏠  Home":
    ...
    stats = [
        ...
        (f"{len(REGISTRATION_STEPS)}", "Registration Steps"),
        (f"{len(FAQ)}", "FAQ Topics"),
    ...
    # Feature cards
    section_title("What You Can Do Here", "Choose a section from the sidebar to get started.")
    ...
    for col, (icon, title, desc) in zip([f1, f2, f3], features):
        with col:
            ...
            <div class="glass-card" style="text-align:center;">
              ...
              <div style="font-size:1.05rem; font-weight:700; color:#f5a623; margin-bottom:8px;">{title}</div>
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div class="glass-card" style="background:rgba(245,166,35,0.06); border-color:rgba(245,166,35,0.25);">
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col_info:
        section_title("Eligibility Criteria", "Requirements you must meet to vote.")
        criteria = [
            ("🎂", f"Minimum Age", f"You must be at least {MINIMUM_VOTING_AGE} years old on or before election day."),
            ("🌍", "Citizenship", "You must be a registered citizen of the country where you wish to vote."),
            ("🏠", "Residency", "Some regions require proof of address within the constituency."),
            ("📋", "Registration", "Being eligible is not enough — you must also be registered to vote."),
        ]
        for icon, title, detail in criteria:
            st.markdown(f"""
            <div class="glass-card" style="padding:16px 20px; margin-bottom:12px;">
              <div style="display:flex; align-items:flex-start; gap:12px;">
                <span style="font-size:1.4rem;">{icon}</span>
                <div>
                  <div style="font-weight:600; color:#cbd5e1; font-size:0.95rem;">{title}</div>
                  <div style="color:#64748b; font-size:0.84rem; margin-top:3px; line-height:1.5;">{detail}</div>
                </div>
              </div>
            </div>
            """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col_steps:
        section_title("Registration Steps", "Check off each step as you complete it.")
        for idx, step in enumerate(REGISTRATION_STEPS, start=1):
            ...
            card_class = "step-card done" if done else "step-card"

            st.markdown(f"""
            <div class="{card_class}">
              <span class="step-num {num_class}">{"✓" if done else idx}</span>
    ...
    with col_sidebar:
        section_title("Quick Tips", "Keep these in mind during registration.")
        tips = [
            ("📅", "Check Deadlines", "Registration typically closes 15–30 days before election day."),
            ("📄", "Prepare Docs", "Have your ID and proof of address ready before starting."),
            ("🔍", "Verify Status", "Always confirm your registration is active after submitting."),
            ("📬", "Absentee Ballot", "Ask about postal voting if you cannot attend in person."),
        ]
        for icon, tip_title, tip_body in tips:
            ...
            <div class="glass-card" style="padding:14px 18px; margin-bottom:12px;">
              ...
              <div style="font-weight:600; color:#f5a623; font-size:0.9rem; margin-bottom:4px;">{tip_title}</div>
              <div style="color:#64748b; font-size:0.83rem; line-height:1.5;">{tip_body}</div>
            </div>
            """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    section_title("Frequently Asked Questions", "Click on any question to expand the answer.")
    ...
    search = st.text_input("🔍  Search questions…", placeholder="Type a keyword…")

    for key, (question, answer) in faq_extended.items():
        if search.lower() in question.lower() or search.lower() in answer.lower() or not search:
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div class="glass-card" style="text-align:center; background:rgba(245,166,35,0.05); border-color:rgba(245,166,35,0.2);">
      <div style="font-size:1.5rem; margin-bottom:8px;">📞</div>
      <div style="font-weight:600; color:#f5a623; margin-bottom:4px;">Still have questions?</div>
      <div style="color:#64748b; font-size:0.9rem;">
        Contact your local election authority or visit the official national election commission website for jurisdiction-specific guidance.
      </div>
    </div>
    """, unsafe_allow_html=True)
